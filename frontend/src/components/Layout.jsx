...
const Layout = ({ onLogOut, user }) => {
  ...
  const fetchTasks = useCallback(async () => {
    ...
    try {
      ...
      const { data } = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/tasks/gp`, {
  ...
  const StatCard = ({ title, value, icon }) => (
    ...
    hover:border-purple-200 group'>
      <div className='flex items-center gap-2'>
        <div className='p-1.5 rounded-lg bg-gradient-to-br from-fuchsia-500/10 to-purple-500/10 group-hover:from-fuchsia-500/20 group-hover:to-purple-500/20 flex items-center justify-center'>
          {icon}
        </div>
